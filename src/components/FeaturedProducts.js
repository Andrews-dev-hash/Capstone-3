...
export default function FeaturedProducts() {
  ...
  useEffect(() => {
    fetch(`http://localhost:4003/b3/products/`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        ...
        for (let i = 0; i < 3; i++) {
          ...
          // For each iteration of the loop, the PreviewProducts component is rendered with the corresponding course data from the data array based on the random number. The key prop is set to the _id of the course for React's reconcilliation to track components
          featured.push(
            <PreviewProducts
              data={data[numbers[i]]}
