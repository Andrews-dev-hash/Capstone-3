...
function CartPage() {
  ...
  const handleRemoveFromCart = async (productId) => {
    try {
      const response = await fetch(
        `http://localhost:4003/b3/users/cart/${productId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleUpdateQuantity = async (productId, newQuantity) => {
    try {
      const response = await fetch(
        `http://localhost:4003/b3/users/cart/addToCart`,
        {
          method: "POST",
          headers: {
  ...
  const fetchCartData = () => {
    fetch(`http://localhost:4003/b3/users/cart/view`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  const handlePlaceOrder = async () => {
    try {
      const response = await fetch(`http://localhost:4003/b3/users/place-order`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      ...
      if (response.ok) {
        const userResponse = await fetch(
          `http://localhost:4003/b3/users/details`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
